...
/* Add function pm8058_gpio_set_value to configure pm gpio */
int pm8058_gpio_set_value( unsigned gpio, int value) 
{ 
	int rc; 
	u8 bank1; 
	...
	mutex_lock(&pm8058_gpio_chip.pm_lock); 
	bank1 = pm8058_gpio_chip.bank1[gpio] & ~PM8058_GPIO_OUT_INVERT; 

	if (value) 
		bank1 |= PM8058_GPIO_OUT_INVERT; 

	pm8058_gpio_chip.bank1[gpio] = bank1; 
	rc = pm8058_write(pm8058_gpio_chip.pm_chip, SSBI_REG_ADDR_GPIO(gpio), &bank1, 1); 
	mutex_unlock(&pm8058_gpio_chip.pm_lock); 
...
EXPORT_SYMBOL(pm8058_gpio_set_value);
