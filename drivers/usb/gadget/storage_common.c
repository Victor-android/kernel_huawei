...
/*<DTS2011032604399-renjun-20110326 begin*/		
#ifndef CONFIG_HUAWEI_KERNEL		
		if (-EROFS == PTR_ERR(filp))
#else
/*
  *this function maybe called by adbd whose identity is shell to open the file /data/cdrom/autorun.iso 
  *whose owner is root and will failed with error number EACCESS,but will succeeded if read only open.
  *if we got the return code eaccess ,try read-only open
  */
		if (-EROFS == PTR_ERR(filp)||-EACCES==PTR_ERR(filp))
#endif
/*DTS2011032604399-renjun-20110326 end>*/
			ro = 1;
	}
	if (ro)
