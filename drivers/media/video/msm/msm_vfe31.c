...
{
	unsigned long flags;
	/*< DTS2011062306109   songxiaoming 20110726 begin */
    CDBG("vfe31_update\n");
	...
	/* DTS2011062306109   songxiaoming 20110726 end > */
	spin_lock_irqsave(&vfe31_ctrl->update_ack_lock, flags);
...
static int vfe31_proc_general(struct msm_vfe31_cmd *cmd)
{
	...
	case V31_MCE_CFG:{
		...
		old_val = msm_io_r(vfe31_ctrl->vfebase + V31_CHROMA_SUP_OFF + 4);//lijuan change
		if (!cmdp) {
			rc = -ENOMEM;
			goto proc_general_done;
		}
    ...
    case V31_LA_UPDATE: {
        ...
        if (copy_from_user(cmdp,(void __user *)(cmd->value),cmd->length)) {
            rc = -EFAULT;
            goto proc_general_done;
        }
