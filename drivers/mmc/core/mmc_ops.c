...
/*< DTS2010073100490 liyuping 20100731 begin */  
/*NOTES: Magic number '0x55AA' is test pattern string.it's a temporary method to avoid boot failed for micron eMMC,we will remove it in future*/
	if (test_pat[0] == bustest_recv_pat[buswidth] || test_pat[0] == 0x55AA)
		pr_debug("%s: Bus test pass for buswidth:%d\n",
						 __func__, buswidth);
/* DTS2010073100490 liyuping 20100731 end >*/
	else
		err = -1;
cmderr:
	kfree(test_pat);
